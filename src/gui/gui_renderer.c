...
void gambar_elemen_otim(EnkiObject* elemen, EnkiObject* gaya_bawaan, EnkiObject* gaya_dev, int* x_kursor, int* y_kursor, Vector2 mouse, bool klik_kiri, char* aksi_kembalian, Color warna_teks_turunan) {
    ...
    char* atribut = ""; char* teks_isi = ""; char* teks_dalam = ""; EnkiObject* anak_anak = NULL;
    ...
    int bingkai_ganda = 0;
    // 🟢 Set nilai awal ke -1 ("Otomatis"). Jika tidak diubah SNUL, wadah pakai 600, tombol pakai auto-text.
    int lebar_dinamis = -1;
    ...
    if (strcmp(tag_nama, "wadah") == 0 || strcmp(tag_nama, "wadah_dinamis") == 0) {
        ...
        Rectangle area = { (float)*x_kursor, (float)*y_kursor, (float)lebar_efektif, (float)tinggi_dinamis };
        ...
        *x_kursor -= 20; 
        ...
        // 🟢 FIX MUTLAK: Hanya baca teks dalam atau teks isi. Hapus Fallback Atribut!
        // 🟢 FIX MUTLAK: Teks Dinamis (Retasan UNUL) HARUS mengalahkan Teks Statis (Bawaan OTIM)
        char* t = (strlen(teks_isi) > 0) ? teks_isi : teks_dalam; 
        if (strlen(t) == 0) { *y_kursor += 15; return; } // Abaikan jika benar-benar kosong!
        ...
        *y_kursor += 45;
        ...
        // 🟢 LEBAR DINAMIS: Ikuti SNUL, atau ukur teks + 40px Padding!
        int tombol_lebar = (lebar_dinamis != -1) ? lebar_dinamis : (text_width + 40);
        
        Rectangle area = { (float)*x_kursor, (float)*y_kursor, (float)tombol_lebar, 40 };
        bool is_hovered = CheckCollisionPointRec(mouse, area);
        
        // Simpan warna asli sebelum ditimpa sihir hover
        ...
        if (is_hovered) {
            ...
            terapkan_aturan_snul(gaya_dev, selektor_hover, &warna_latar, &warna_teks, &bingkai_ganda, &lebar_dinamis);
            
            char tag_hover[128]; snprintf(tag_hover, sizeof(tag_hover), "@%s:hover", tag_nama);
            terapkan_aturan_snul(gaya_dev, tag_hover, &warna_latar, &warna_teks, &bingkai_ganda, &lebar_dinamis);
        }
        
        Color warna_final = warna_latar;
        
        if (is_hovered) {
            // 🟢 ANTI-ANOMALI: Jika dev tidak mengatur warna latar di :hover, baru kita pudar otomatis!
        ...
        // 🟢 TEKS OTOMATIS BERADA DI TENGAH TOMBOL!
        int x_tengah = *x_kursor + (tombol_lebar / 2) - (text_width / 2);
        DrawText(label_tombol, x_tengah, *y_kursor + 10, 20, w_teks); 
        *y_kursor += 55;
    }
}
...
char* tampilkan_gui_raylib(EnkiObject* ui_root, EnkiObject* gaya_root) {
    ...
    // 🟢 Reset kursor ke panah setiap frame, akan ditimpa jika hover tombol!
